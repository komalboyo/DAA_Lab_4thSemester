...
void generate_permutations(int** cm,int* in,int n,int* mincost,int* minarr,int i, int * opcount)
{
    //printf("hello\n");
    if(i==n)
    {
    ...
    else
    {
        for(int j=i;j<n;j++)
        {
            //(*opcount)++;
            swap(&in[i],&in[j]);
            generate_permutations(cm,in,n,mincost,minarr,i+1, opcount);
            swap(&in[i],&in[j]);
        }
    }
}
...
void main()
{
    ...
    int minarr[n];
    int opcount=0;
    generate_permutations(costmatrix,ind,n,&mincost,minarr,0, &opcount);
